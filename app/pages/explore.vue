<template>
	<div class="flex-1 flex flex-col h-full bg-[var(--background-gray-main)] overflow-hidden transition-colors">
		<!-- Header: Desktop -->
		<header class="hidden lg:flex bg-[var(--bg-main)] px-6 py-3 items-center justify-between gap-6 shrink-0 z-10">
		...
		<div :class="['flex-1 overflow-y-auto px-4 lg:px-5 pb-12 custom-scrollbar', showMyCreated ? 'pt-4' : '']" ref="scrollContainer" @scroll="handleScroll">
			<div class="w-full min-h-full mt-5">
				...
				<div v-if="isListLoading && displayList.length === 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-2.5 sm:gap-3.5">
				...
				<!-- Bot Cards Grid -->
				<div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-2.5 sm:gap-3.5">
					<div v-for="bot in displayList" :key="bot.id" :data-key="bot.id" class="relative flex items-center gap-3.5 px-4 py-3.5 rounded-2xl bg-[var(--background-white-main)] border border-[var(--border-light)] hover:border-[var(--border-main)] hover:bg-[#f0f0ee] transition-all cursor-pointer group" @click="handleBotClick(bot)">
						...
						<!-- Favorite -->
						<button v-if="!showMyCreated" @click.stop="toggleFavorite(Number(bot.id))" class="flex-shrink-0 size-8 flex items-center justify-center rounded-full bg-black/40 backdrop-blur-sm hover:bg-black/60 opacity-0 group-hover:opacity-100 transition-all outline-none border-none" :class="isFavorited(Number(bot.id)) ? '!opacity-100' : ''">
							<Heart :size="16" :class="isFavorited(Number(bot.id)) ? 'fill-red-500 !stroke-transparent' : 'fill-white/80 !stroke-transparent'" />
						</button>
					</div>
				</div>

				<div v-if="hasFetched && !isListLoading && displayList.length === 0" class="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] gap-1">
					...
					<button v-if="showMyCreated" @click="router.push('/character/create')" class="mt-4 px-5 py-2 rounded-full bg-[var(--text-primary)] text-[var(--bg-main)] text-[13px] font-semibold hover:opacity-90 transition-opacity">
						{{ $t('explore.create_bot') }}
					</button>
					<button v-else @click="retryFetchDiscovery" class="mt-4 px-5 py-2 rounded-full bg-[var(--text-primary)] text-[var(--bg-main)] text-[13px] font-semibold hover:opacity-90 transition-opacity flex items-center gap-2">
						<RefreshCw :size="14" />
						{{ $t('chat.retry') }}
					</button>
				</div>
...
const tryAutoFill = async () => {
	...
	if (!el) return
	while (discoveryStore.hasMore && !discoveryStore.isLoadingMore && !discoveryStore.isLoading && el.scrollHeight <= el.clientHeight + 200) {
...
const retryFetchDiscovery = () => {
	discoveryStore.hasMore = true
	discoveryStore.fetchDiscovery(selectedTag.value, true)
}
