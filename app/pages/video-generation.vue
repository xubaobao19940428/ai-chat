<template>
		...
		<main class="flex-1 overflow-y-auto px-4 py-6 pb-56 custom-scrollbar relative">
			<div class="max-w-full mx-auto">
				...
				<div v-else>
					...
					<div v-if="generatedVideos.length === 0 && activeTasks.length === 0"
						...
						<div class="relative group cursor-default">
							...
							<div
								...
								<svg viewBox="0 0 40 40"
									...
									<path d="M18 16L24 20L18 24" stroke-width="1.5" stroke-linecap="round"
					...
					<div v-if="generatedVideos.length > 0 || activeTasks.length > 0"
						...
						<div v-for="task in activeTasks" :key="task.id"
							...
							<div
								class="relative aspect-video flex items-center justify-center overflow-hidden bg-black">
								<!-- Premium Placeholder Background -->
								<img v-if="!task.videoUrl" src="/noir_placeholder.png"
									class="absolute inset-0 w-full h-full object-cover opacity-40 mix-blend-luminosity" />

								<!-- Precision Scanner (Theme Aware) -->
								<div v-if="!task.videoUrl"
								...
								<template v-if="task.videoUrl">
									<img v-if="task.videoUrl.startsWith('data:image')" :src="task.videoUrl"
										class="w-full h-full object-cover animate-noir-reveal relative z-20" />
									<video v-else :src="task.videoUrl"
										class="w-full h-full object-cover animate-noir-reveal relative z-20" autoplay
						...
						<div v-for="(video, index) in filteredGeneratedVideos" :key="video.id"
							...
							<div class="relative overflow-hidden aspect-video bg-[var(--bg-hover)]">
								<template v-if="video.status === 1">
									<img v-if="!playingVideoId || playingVideoId !== video.id"
										:src="video.thumbnail || getRecordPrimaryUrl(video)"
										class="w-full h-full object-cover transform transition-transform duration-700 group-hover:scale-110" />
									<video v-else :src="getRecordPrimaryUrl(video)" class="w-full h-full object-cover"
								...
								<div v-else
									class="h-full relative flex flex-col items-center justify-center overflow-hidden bg-black/40">
									...
									<div class="relative z-10 flex flex-col items-center gap-4">
										...
										<span
											class="text-[9px] font-black text-white/40 tracking-[0.3em] uppercase">Processing</span>
									</div>
								</div>
							</div>
							...
							<div
								...
								<div
									...
									<div v-if="getRecordParams(video)" class="flex flex-wrap gap-2 mb-4 opacity-70">
										<div v-if="getRecordParams(video).aspect_ratio"
										...
										<div v-if="getRecordParams(video).resolution"
											class="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[8px] font-mono text-white/80 uppercase">
											{{ getRecordParams(video).resolution }}
										</div>
										<div v-if="getRecordParams(video).duration"
											class="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[8px] font-mono text-white/80 uppercase">
											{{ getRecordParams(video).duration }}S</div>
										<div v-if="getRecordParams(video).seed"
											class="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[8px] font-mono text-white/80 uppercase">
											SEED: {{ getRecordParams(video).seed }}</div>
									</div>

									<p
										class="text-white text-[13px] font-medium mb-4 line-clamp-3 leading-relaxed italic">
										"{{
											getRecordPrompt(video) || 'No description' }}"</p>
					...
					<div class="flex w-full min-w-0 flex-col gap-5">
						...
						<div
							...
							<textarea v-model="prompt"
								class="w-full bg-transparent border-none outline-none font-normal resize-none px-0 text-inherit leading-inherit"
								:placeholder="displayedPlaceholder" autocomplete="off" rows="2"
						...
						<div class="flex justify-between gap-2 px-3 mt-auto">
							...
							<div class="flex items-center gap-1 flex-wrap">
								...
								<div class="group/button relative" v-if="supportsImageUpload">
									<button @click="toggleStartFrameDropdown"
									...
									<div v-if="openStartFrameDropdown"
										...
										<div class="rounded-2xl bg-[var(--bg-main)] border border-[var(--border-light)] p-4 shadow-lg min-w-[300px] flex flex-col gap-4"
											...
											<p
												...
												select
												one from your assets.</p>

											<div class="flex flex-col gap-2">
												<!-- Upload Button -->
												<button @click="handleStartFrameUpload"
												...
												<!-- Select Asset Button -->
												<button @click="handleSelectAsset"
							...
							<!-- Generate Button -->
							<button @click="generateVideo" :disabled="!prompt.trim()"
	...
	<!-- Asset Picker Modal -->
	<AssetPickerModal :show="showAssetPicker" :multiple="false" file-type="image" @close="showAssetPicker = false"
