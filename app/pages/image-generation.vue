			...
			<div class="max-w-full mx-auto">
				...
				<div v-else>
					...
					<div v-if="generatedImages.length === 0" class="py-40 flex flex-col items-center justify-center">
						...
						<p class="text-[var(--text-tertiary)] text-[15px] mt-2 max-w-[280px] text-center leading-relaxed">Transform your ideas into stunning visuals. Every creation will be preserved here.</p>
			...
			<div class="w-full max-w-[840px] relative pointer-events-auto">
				<transition enter-active-class="duration-300 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0">
					<div v-if="isGenerating" class="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-1.5 bg-[var(--bg-main)] border border-[var(--border-main)] rounded-full shadow-lg">
						...
						<span class="text-[11px] font-bold text-[var(--text-primary)]">{{ generationProgress }}% Generating...</span>
					</div>
				</transition>
				...
				<div ref="controlBarRef" class="bg-[var(--bg-main)] rounded-[40px] border border-[var(--border-main)] p-5 shadow-[var(--shadow-pill)] flex flex-col gap-3 transition-all">
					...
					<div class="flex items-center justify-between gap-2 px-1">
						...
						<!-- Generate Button -->
						<button @click="generateImage" :disabled="!prompt.trim() || isGenerating" class="flex items-center justify-center shrink-0 w-10 h-10 rounded-full bg-[var(--text-primary)] text-[var(--bg-main)] hover:opacity-90 transition-all disabled:opacity-50 disabled:pointer-events-none self-end">
