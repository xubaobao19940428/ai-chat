					...
					<MasonryGrid v-else :items="exampleImages" v-slot="{ item: example }">
						<div class="group relative rounded-2xl overflow-hidden bg-white dark:bg-[var(--background-card)] border border-[var(--border-main)] hover:border-[var(--text-tertiary)] transition-all duration-300 cursor-pointer shadow-sm hover:shadow-md h-fit"
							...
							<div
								...
								class="text-white text-[13px] line-clamp-5">{{ example.description
								}}</p>
										<a v-if="example.author" :href="example.original_url || `https://x.com/${example.author}`" target="_blank" rel="noopener noreferrer" @click.stop class="text-white/50 text-[11px] mt-0.5 hover:text-white/80 hover:underline transition-colors pointer-events-auto block">@{{
									example.author }}</a>
									</div>
									<!-- Right: Use prompt -->
									<span
