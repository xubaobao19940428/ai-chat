	...
	<div class="flex-1 flex flex-col h-full bg-[var(--bg-main)] overflow-hidden relative transition-colors">
		...
		<div v-else class="flex-1 overflow-hidden flex flex-col">
			...
			<main class="flex-1 flex flex-col h-full bg-[var(--bg-main)] relative overflow-hidden">
						...
						<div class="space-y-6">
							<div v-for="message in storeMessages" :key="message.id" class="flex group" :class="message.role === 'user' ? 'justify-end' : ''">
								...
								<div class="flex flex-col max-w-[85%] sm:max-w-[80%]" :class="message.role === 'user' ? 'items-end' : 'items-start'">
									<div v-if="message.role === 'user'" class="bg-[var(--bg-chat-bubble-user)] text-[var(--text-primary)] px-5 py-2.5 rounded-[16px] text-[15px] font-medium leading-relaxed border border-[var(--border-light)]">
				...
				<!-- Floating Input Area -->
				<div class="flex-shrink-0 px-4 pt-2 absolute left-0 right-0 z-50 pointer-events-none" :style="{ bottom: keyboardHeight > 0 ? `${keyboardHeight + 8}px` : '0px', paddingBottom: keyboardHeight > 0 ? '8px' : '12px' }">
					<div class="max-w-[840px] mx-auto relative pointer-events-auto">
						...
						<UnifiedInput ref="unifiedInputRef" capability="chat" :is-loading="isStreaming" :character-name="character?.name" :show-model-selector="true" @send="handleUnifiedSend" @stop="stopStreaming" />
					</div>
				</div>
			</main>
		</div>
