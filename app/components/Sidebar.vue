			...
			<div class="flex flex-col flex-1 min-h-0 -mx-[8px] transition-all overflow-hidden" style="opacity: 1">
				...
				<div class="flex flex-col flex-1 min-h-0 overflow-x-hidden overflow-y-auto custom-scrollbar pt-0 px-[8px]">
					...
					<div class="flex flex-col gap-px">
						<div v-if="conversationStore.isLoading && conversationStore.conversations.length === 0" class="flex flex-col gap-2 px-2 py-1">
							<div v-for="i in 5" :key="i" class="h-8 w-full bg-[var(--bg-hover)] rounded-lg animate-pulse"></div>
						</div>
						<div v-else v-for="conversation in sortedConversations" :key="conversation.id" @click="handleSelectConversation(String(conversation.id))" :class="['group flex items-center rounded-[10px] clickable cursor-pointer transition-all w-full gap-[12px] h-[36px] ps-[9px] pe-[2px]', conversationStore.currentConversationId == conversation.id ? 'bg-[var(--bg-hover)]' : 'hover:bg-[var(--bg-hover)]']">
								...
								<Menu as="div" class="relative inline-block text-left">
									...
									<Teleport to="body">
										<transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
											<MenuItems v-if="activeMenuId === conversation.id" class="fixed w-56 origin-top-right divide-y divide-[var(--border-light)] rounded-[12px] bg-[var(--background-white-main)] shadow-[var(--shadow-XL)] border border-[var(--border-main)] focus:outline-none z-[100] overflow-hidden" :style="menuPosition">
												<div class="px-1 py-1">
												...
												<div class="px-1 py-1">
													...
													<div class="relative group/submenu">
														...
														<div class="absolute left-full top-0 ml-0 hidden group-hover/submenu:block w-48 rounded-[12px] bg-[var(--background-white-main)] shadow-[var(--shadow-XL)] border border-[var(--border-main)] focus:outline-none z-[60] p-1">
															<button @click="handleMoveToProject(conversation.id, 0)" class="flex w-full items-center gap-3 rounded-[8px] px-3 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--fill-tsp-white-main)] transition-colors">Direct Chat</button>
															<button v-for="proj in projectStore.projects" :key="proj.id" @click="handleMoveToProject(conversation.id, proj.id)" class="flex w-full items-center gap-3 rounded-[8px] px-3 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--fill-tsp-white-main)] transition-colors">
																{{ proj.name }}
															</button>
														</div>
													</div>
