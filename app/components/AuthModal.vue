								...
								<div class="relative z-10 flex-1 flex flex-col">
									...
									<div class="flex flex-col items-center text-center mt-4">
										...
										<div class="inline-flex flex-col items-center mb-6">
											...
											<div class="mt-4 px-4 py-1.5 bg-white dark:bg-gray-800 rounded-full border border-gray-200 dark:border-gray-700 shadow-sm text-xs font-semibold text-gray-600 dark:text-gray-300">Available on</div>
										</div>
									...
									<div class="relative mt-auto overflow-hidden">
										<div class="flex gap-4 animate-scroll whitespace-nowrap py-4">
											...
											<div v-for="group in 3" :key="group" class="flex gap-4">
												<div v-for="t in testimonials" :key="t.name" class="inline-block w-64 p-5 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm whitespace-normal">
													<div class="flex gap-0.5 mb-2">
														<svg v-for="i in 5" :key="i" class="w-3 h-3 text-yellow-400 fill-current" viewBox="0 0 20 20">
									...
									<form @submit.prevent="authStep === 'email' ? handleCheckEmail() : handleAuth()">
										<div v-if="authStep === 'email'" class="space-y-4">
											<div>
												...
												<input type="email" id="email" v-model="email" required class="w-full px-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20 text-gray-900 dark:text-white focus:border-indigo-500 dark:focus:border-indigo-500 outline-none transition-all placeholder:text-gray-400 font-medium" placeholder="Enter your email" />
											</div>
											<button type="submit" class="w-full py-4 px-6 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-white rounded-2xl font-bold transition-all">Continue with Email</button>
										</div>
										...
										<div v-else class="space-y-4">
											...
											<div v-if="isRegistering" class="flex gap-2 mb-4">
												<div class="flex-1">
													<input type="text" v-model="code" required class="w-full px-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20 text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all font-medium" placeholder="Verification Code" />
												</div>
												<button type="button" @click="handleSendCode" :disabled="countdown > 0 || sendingCode" class="px-5 py-4 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-2xl font-bold transition-all disabled:opacity-50 whitespace-nowrap min-w-[120px]">
													{{ countdown > 0 ? `${countdown}s` : 'Send Code' }}
												</button>
											</div>

											<div>
												<input type="password" v-model="password" required class="w-full px-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20 text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all font-medium" placeholder="Password" />
											</div>
