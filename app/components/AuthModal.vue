			...
			<div class="fixed inset-0 z-10 overflow-y-auto">
				<div class="flex min-h-full items-center justify-center sm:p-4 text-center sm:items-center">
					<Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to-class="opacity-100 translate-y-0 sm:scale-100" leave-active-class="transition ease-in duration-200" leave-from-class="opacity-100 translate-y-0 sm:scale-100" leave-to-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
						<div class="relative transform bg-[var(--background-white-main)] text-left shadow-[var(--shadow-XL)] transition-all w-full h-screen sm:h-auto sm:max-h-[95vh] sm:my-4 sm:w-full sm:max-w-5xl rounded-[16px] flex flex-col sm:flex-row overflow-hidden border border-[var(--border-main)]">
							<!-- Close Button -->
							<button @click="uiStore.closeLoginModal" class="absolute top-6 right-6 z-50 p-2 rounded-full bg-[var(--background-white-main)] border border-[var(--border-main)] hover:bg-[var(--fill-tsp-white-main)] transition-all shadow-sm group">
								<svg class="w-5 h-5 text-[var(--icon-secondary)] group-hover:text-[var(--text-primary)] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									...
									<div class="relative mt-auto overflow-hidden">
										<div class="flex gap-4 animate-scroll whitespace-nowrap py-6">
											...
											<div v-for="group in 3" :key="group" class="flex gap-4">
												<div v-for="t in testimonials" :key="t.name" class="inline-block w-72 p-6 bg-[var(--background-tsp-menu-white)] backdrop-blur-xl rounded-[16px] border border-[var(--border-white)] shadow-[var(--shadow-M)] whitespace-normal hover:-translate-y-1 transition-transform">
													...
													<div class="flex items-center gap-3">
														<div class="w-10 h-10 rounded-xl bg-[var(--fill-blue)] flex items-center justify-center text-sm font-bold text-[var(--text-blue)]">
															{{
																t.name
																	.split(' ')
																	.map((n) => n[0])
							...
							<div class="flex-1 flex flex-col p-8 sm:p-12 justify-center bg-[var(--background-white-main)] relative">
								<div class="max-w-md mx-auto w-full relative z-10">
									...
									<div class="text-center mb-10">
										...
										<div class="flex items-center justify-center gap-8 py-3 px-6 bg-gray-50/50 dark:bg-gray-900/30 backdrop-blur-sm rounded-full border border-gray-100/50 dark:border-gray-800/50">
									...
									<div class="space-y-3 mb-8">
										<button class="w-full flex items-center justify-center gap-3 px-4 py-4 bg-[var(--Button-primary-black)] text-[var(--text-onblack)] rounded-[10px] hover:opacity-90 transition-all font-bold shadow-[var(--shadow-S)] hover:scale-[1.01] active:scale-[0.99]">
											<svg class="w-5 h-5" viewBox="0 0 24 24">
												<path fill="currentColor" d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z" />
											</svg>
											Continue with Google
										</button>
										<button class="w-full flex items-center justify-center gap-3 px-4 py-4 border-2 border-[var(--border-main)] rounded-[10px] hover:bg-[var(--fill-tsp-white-main)] transition-all bg-[var(--background-white-main)] text-[var(--text-primary)] font-bold hover:scale-[1.01] active:scale-[0.99]">
										...
										<Transition enter-active-class="transition ease-out duration-300 transform" enter-from-class="opacity-0 translate-x-4" enter-to-class="opacity-100 translate-x-0" leave-active-class="transition ease-in duration-200 transform absolute" leave-from-class="opacity-100 translate-x-0" leave-to-class="opacity-0 -translate-x-4">
											<div v-if="authStep === 'email'" key="email-step" class="space-y-4">
												<div class="relative group">
													...
													<input type="email" id="email" v-model="email" required class="w-full pl-12 pr-5 py-4 rounded-[10px] border-2 border-[var(--border-main)] bg-[var(--fill-tsp-white-main)] text-[var(--text-primary)] focus:border-[var(--border-blue)] outline-none transition-all placeholder:text-[var(--text-disable)] font-bold" placeholder="Enter your email" />
												</div>
												<button type="submit" class="w-full py-4 px-6 bg-[var(--Button-primary-black)] text-[var(--text-onblack)] rounded-[10px] font-bold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-sm">Continue with Email</button>
											</div>
											...
											<div v-else key="auth-step" class="space-y-4">
												...
												<div v-if="isRegistering" class="flex gap-2 mb-4">
													<div class="relative flex-1 group">
														...
														<input type="text" v-model="code" required class="w-full pl-12 pr-5 py-4 rounded-[10px] border-2 border-[var(--border-main)] bg-[var(--fill-tsp-white-main)] text-[var(--text-primary)] focus:border-[var(--border-blue)] outline-none transition-all font-bold" placeholder="Verification Code" />
													</div>
													<button type="button" @click="handleSendCode" :disabled="countdown > 0 || sendingCode" class="px-5 py-4 bg-[var(--Button-secondary-gray)] text-[var(--text-primary)] rounded-[10px] font-bold transition-all disabled:opacity-50 whitespace-nowrap min-w-[120px] hover:bg-[var(--fill-tsp-gray-main)]">
												...
												<div class="relative group">
													...
													<input :type="showPassword ? 'text' : 'password'" v-model="password" required class="w-full pl-12 pr-12 py-4 rounded-[10px] border-2 border-[var(--border-main)] bg-[var(--fill-tsp-white-main)] text-[var(--text-primary)] focus:border-[var(--border-blue)] outline-none transition-all font-bold" placeholder="Password" />
												...
												<button type="submit" :disabled="loading" class="w-full py-4 px-6 bg-[var(--Button-primary-brand)] text-[var(--text-onblack)] rounded-[10px] font-bold transition-all disabled:opacity-50 shadow-sm hover:scale-[1.02] active:scale-[0.98]">
