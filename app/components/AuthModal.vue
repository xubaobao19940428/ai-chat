				...
				<div class="flex min-h-full items-center justify-center sm:p-4 text-center sm:items-center">
					<Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to-class="opacity-100 translate-y-0 sm:scale-100" leave-active-class="transition ease-in duration-200" leave-from-class="opacity-100 translate-y-0 sm:scale-100" leave-to-class="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
						<div class="relative transform bg-white dark:bg-[#1a1a1a] text-left shadow-xl transition-all w-full h-screen sm:h-auto sm:max-h-[95vh] sm:my-4 sm:w-full sm:max-w-5xl sm:rounded-3xl flex flex-col sm:flex-row overflow-hidden border border-gray-100 dark:border-gray-800">
							...
							<div class="hidden sm:flex flex-[1.2] flex-col bg-gradient-to-br from-[#fcfcfd] via-indigo-50/20 to-violet-50/20 dark:from-black dark:via-indigo-950/10 dark:to-violet-950/10 p-10 relative overflow-hidden border-r border-gray-100 dark:border-gray-900/50">
								...
								<div class="relative z-10 flex-1 flex flex-col">
									...
									<div class="flex flex-col items-center text-center mt-4">
										...
										<div class="inline-flex flex-col items-center mb-6">
											...
											<div class="text-xl font-bold text-gray-900 dark:text-white tracking-tight">#1 AI Chatbot</div>
											<div class="text-4xl font-black bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 mt-1">35M+ users</div>
											<div class="mt-6 px-4 py-1.5 bg-white/50 dark:bg-gray-800/50 backdrop-blur-md rounded-full border border-gray-200/50 dark:border-gray-700/50 shadow-sm text-xs font-bold text-gray-600 dark:text-gray-300">Available on</div>
										</div>
									...
									<div class="relative mt-auto overflow-hidden">
										<div class="flex gap-4 animate-scroll whitespace-nowrap py-6">
											...
											<div v-for="group in 3" :key="group" class="flex gap-4">
												<div v-for="t in testimonials" :key="t.name" class="inline-block w-72 p-6 bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl rounded-3xl border border-white/20 dark:border-white/5 shadow-xl shadow-gray-200/20 dark:shadow-none whitespace-normal hover:-translate-y-1 transition-transform">
													...
													<div class="flex items-center gap-3">
														<div class="w-10 h-10 rounded-2xl bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-sm font-bold text-indigo-600 dark:text-indigo-400">
															{{
																t.name
																	.split(' ')
																	.map((n) => n[0])
														...
														<div class="text-left">
															<div class="text-xs font-bold text-gray-900 dark:text-white">{{ t.name }}</div>
															<div class="text-[10px] text-gray-500 font-medium">{{ t.role }}</div>
														</div>
													</div>
												</div>
											</div>
							...
							<div class="flex-1 flex flex-col p-8 sm:p-12 justify-center bg-white dark:bg-[#1a1a1a] relative">
								<div class="max-w-md mx-auto w-full relative z-10">
									...
									<div class="text-center mb-10">
										<h2 class="text-3xl font-black text-gray-900 dark:text-white leading-tight tracking-tight">Experience Cosmic Intelligence</h2>
										<p class="mt-3 text-sm text-gray-500 dark:text-gray-400 font-medium">Step into the future of human-AI collaboration.</p>
									</div>

									<!-- Provider Buttons -->
									<div class="space-y-3 mb-8">
										<button class="w-full flex items-center justify-center gap-3 px-4 py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl hover:opacity-90 transition-all font-bold shadow-xl shadow-gray-200 dark:shadow-none hover:scale-[1.01] active:scale-[0.99]">
											<svg class="w-5 h-5" viewBox="0 0 24 24">
												<path fill="currentColor" d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z" />
											</svg>
											Continue with Google
										</button>
										<button class="w-full flex items-center justify-center gap-3 px-4 py-4 border-2 border-gray-100 dark:border-gray-800 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all bg-white dark:bg-transparent text-gray-700 dark:text-white font-bold hover:scale-[1.01] active:scale-[0.99]">
									...
									<div class="relative my-10">
										...
										<div class="relative flex justify-center text-[10px] font-black uppercase tracking-[0.3em]">
											<span class="px-6 bg-white dark:bg-[#1a1a1a] text-gray-400">or use email</span>
										</div>
									</div>
										...
										<Transition enter-active-class="transition ease-out duration-300 transform" enter-from-class="opacity-0 translate-x-4" enter-to-class="opacity-100 translate-x-0" leave-active-class="transition ease-in duration-200 transform absolute" leave-from-class="opacity-100 translate-x-0" leave-to-class="opacity-0 -translate-x-4">
											<div v-if="authStep === 'email'" key="email-step" class="space-y-4">
												<div class="relative group">
													...
													<input type="email" id="email" v-model="email" required class="w-full pl-12 pr-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20 text-gray-900 dark:text-white focus:border-indigo-500 dark:focus:border-indigo-500 outline-none transition-all placeholder:text-gray-400 font-medium" placeholder="Enter your email" />
												</div>
												<button type="submit" class="w-full py-4 px-6 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-bold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-gray-200 dark:shadow-none">Continue with Email</button>
											</div>

											<div v-else key="auth-step" class="space-y-4">
												<div class="text-left mb-2">
													<button type="button" @click="authStep = 'email'" class="px-3 py-1 text-xs font-bold bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg hover:opacity-80 flex items-center mb-4 transition-all">
												...
												<div v-if="isRegistering" class="flex gap-2 mb-4">
													<div class="relative flex-1 group">
														...
														<input type="text" v-model="code" required class="w-full pl-12 pr-5 py-4 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20 text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all font-medium" placeholder="Verification Code" />
													</div>
													<button type="button" @click="handleSendCode" :disabled="countdown > 0 || sendingCode" class="px-5 py-4 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-2xl font-bold transition-all disabled:opacity-50 whitespace-nowrap min-w-[120px] hover:bg-gray-200 dark:hover:bg-gray-700">
												...
												<div class="relative group">
													...
													<input :type="showPassword ? 'text' : 'password'" v-model="password" required class="w-full pl-12 pr-12 py-4 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-black/20 text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all font-medium" placeholder="Password" />
													<button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
												...
												<button type="submit" :disabled="loading" class="w-full py-4 px-6 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold transition-all disabled:opacity-50 shadow-lg shadow-indigo-200 dark:shadow-none hover:scale-[1.02] active:scale-[0.98]">
												...
												<div class="text-center mt-6">
													<button type="button" @click="toggleAuthMode" class="text-sm font-bold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 decoration-2 hover:underline underline-offset-4 transition-all">
