		...
		<button v-if="variant === 'ghost'" @click="isOpen = !isOpen" class="flex h-8 px-2 justify-center items-center gap-1.5 rounded-[8px] clickable hover:bg-[var(--fill-tsp-gray-main)] transition-colors transition-all duration-300 -ml-2 cursor-pointer group">
			<ClientOnly>
				<div class="flex items-center gap-2 max-w-[200px]">
					...
					<span class="text-[var(--text-primary)] md:text-[18px] text-[16px] font-medium md:leading-[22px] leading-[20px] truncate tracking-tight">{{ modelStore.selectedModel?.display_name || $t('common.select_model') }}</span>
				</div>
			</ClientOnly>
		...
		<button v-else-if="variant === 'pill'" @click="isOpen = !isOpen" class="flex items-center gap-1.5 rounded-full px-2.5 h-[32px] text-[var(--text-secondary)] hover:bg-[var(--fill-tsp-gray-main)] transition-colors cursor-pointer border border-transparent hover:border-[var(--border-main)] group">
			<ClientOnly>
				<div class="flex items-center gap-1.5">
					...
					<span class="text-xs font-medium">{{ modelStore.selectedModel?.display_name || $t('common.select_model') }}</span>
				</div>
			</ClientOnly>
		...
		<button v-else @click="isOpen = !isOpen" class="flex items-center gap-2.5 px-3 py-2 bg-[var(--fill-tsp-white-main)] hover:bg-[var(--fill-tsp-white-light)] text-[var(--text-primary)] rounded-xl transition-all text-[11px] font-bold cursor-pointer border border-[var(--border-main)] active:scale-95 shadow-[var(--shadow-XS)] group">
			<ClientOnly>
				<div class="flex items-center gap-2 max-w-[140px] sm:max-w-[170px]">
					...
					<span class="truncate">{{ modelStore.selectedModel?.display_name || $t('common.select_model') }}</span>
				</div>
			</ClientOnly>
					...
					<div v-else>
						...
						<div class="p-2 border-b border-[var(--border-main)]">
							<div class="flex items-center gap-2 px-2.5 py-1.5 rounded-[8px] bg-[var(--background-gray-main)]">
								...
								<input ref="searchInputRef" v-model="searchQuery" type="text" :placeholder="$t('common.search_models')" class="flex-1 bg-transparent text-[13px] text-[var(--text-primary)] placeholder:text-[var(--text-disable)] outline-none" />
						...
						<div class="max-h-[320px] overflow-y-auto no-scrollbar p-1.5 space-y-0.5">
							<template v-if="filteredModels.length">
								<button v-for="model in filteredModels" :key="model.model" @click="selectModel(`${model.provider}:${model.model}`)" class="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-[8px] transition-colors text-left group" :class="isSelected(model) ? 'bg-[var(--fill-blue)]' : 'hover:bg-[var(--fill-tsp-white-main)]'">
									<!-- Icon -->
									...
									<div class="flex-1 min-w-0">
										<div class="text-[13px] font-medium leading-tight truncate" :class="isSelected(model) ? 'text-[var(--text-blue)]' : 'text-[var(--text-primary)]'">
											{{ model.display_name }}
										</div>
										<div class="text-[11px] leading-tight mt-0.5 truncate" :class="isSelected(model) ? 'text-[var(--text-blue)]/70' : 'text-[var(--text-tertiary)]'">
