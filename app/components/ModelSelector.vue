				...
				<div class="flex items-center gap-2 max-w-[200px]">
					...
					<span
						class="text-[var(--text-primary)] md:text-[18px] text-[16px] font-medium md:leading-[22px] leading-[20px] truncate tracking-tight">{{
							modelStore.selectedModel?.display_name || 'Select Model' }}</span>
				</div>
			</ClientOnly>
		...
		<button v-else-if="variant === 'pill'" @click="isOpen = !isOpen"
			...
			<ClientOnly>
				<div class="flex items-center gap-1.5">
					...
					<span class="text-xs font-medium">{{ modelStore.selectedModel?.display_name || 'Select Model'
					}}</span>
				</div>
			</ClientOnly>
				...
				<div v-if="isOpen" :style="dropdownStyle"
					...
					<div v-else>
						...
						<div class="max-h-[320px] overflow-y-auto no-scrollbar p-1.5 space-y-0.5">
							...
							<div v-else class="py-6 text-center text-[12px] text-[var(--text-tertiary)]">No models found
							</div>
						</div>
					</div>
				</div>
...
watch(() => filteredModels.value, (newModels) => {
