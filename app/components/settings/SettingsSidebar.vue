<template>
	<div
		class="w-full md:w-[221px] flex-shrink-0 md:border-r border-[var(--border-main)] relative flex flex-col bg-[var(--background-gray-main)]">
		<!-- Mobile Header -->
		<div class="flex md:hidden items-center justify-between px-4 py-3 border-b border-[var(--border-main)]">
			...
			<button @click="$emit('close')"
				class="p-1.5 rounded-lg hover:bg-[var(--fill-tsp-gray-main)] text-[var(--icon-tertiary)] transition-colors">
				<X :size="20" />
			</button>
		</div>
		...
		<nav class="flex-1 overflow-y-auto px-2 pt-2 md:pt-0 space-y-0.5 custom-scrollbar">
			<button v-for="item in menuItems" :key="item.value" @click="$emit('select', item.value)"
				:class="['w-full flex items-center gap-3 px-3 py-2.5 md:py-2 rounded-[10px] md:rounded-[8px] text-[15px] md:text-[14px] font-medium transition-colors', item.value === modelValue ? 'bg-[var(--fill-tsp-white-main)] font-semibold text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:bg-[var(--fill-tsp-white-main)] hover:text-[var(--text-primary)]']">
