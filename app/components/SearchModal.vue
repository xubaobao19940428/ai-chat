      ...
      <div class="fixed inset-0 overflow-y-auto">
        <div class="flex min-h-full items-start justify-center p-4 text-center pt-[120px]">
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
            <DialogPanel class="w-full max-w-[640px] transform overflow-hidden rounded-[16px] bg-[var(--background-white-main)] border border-[var(--border-main)] text-left align-middle shadow-[var(--shadow-XL)] transition-all flex flex-col">
              ...
              <div class="p-2 max-h-[60vh] overflow-y-auto custom-scrollbar">
                ...
                <div v-else class="space-y-4 px-1 pb-2">
                    <div v-for="(items, category) in groupedResults" :key="category">
                        <div class="text-[12px] font-medium text-[var(--text-tertiary)] mb-2 px-2 capitalize">{{ category }}</div>
                        <div class="space-y-0.5">
                            ...
                            >
                                <div class="mt-1 w-5 h-5 flex items-center justify-center text-[var(--text-tertiary)] group-hover:text-[var(--text-primary)]">
                                    <div class="w-5 h-5 rounded-full bg-black flex items-center justify-center">
                                       <span class="text-[10px] text-white font-bold">{{ (item.title && item.title.length > 0) ? item.title[0].toUpperCase() : 'M' }}</span> 
                                    </div>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <span class="text-[14px] text-[var(--text-primary)] font-medium truncate">{{ item.title || 'Empty Chat' }}</span>
                                        <span class="text-[12px] text-[var(--text-tertiary)]">{{ formatTime(item.updatedAt) }}</span>
                                    </div>
                                    <p v-if="item.lastMessage" class="text-[13px] text-[var(--text-tertiary)] truncate">{{ item.lastMessage }}</p>
                                </div>
                            </button>
                        </div>
                    </div>
